...
OBJ_DETECTION_MODEL_PATH = os.path.join(project_dir,'models','yolo_anamoly_detection.pt')

TEMP_TEXT_FILE = os.path.join(current_dir,'alerts.txt')

PREDICTION_THRESHOLD = 99 #percent
TIME_THRESHOLD = 60 #sec
CONFIDENCE_THRESHOLD = 0.383 #percent
...
def detect_object(frame, anamoly):
    # Run inference
    print(f"🔍 Detecting {anamoly} in the frame...")
    results = det_model(frame,conf=CONFIDENCE_THRESHOLD)[0]  # batch size 1, so take first 
    
    if anamoly == 'Blood Detected':
        anamoly = 'blood'
    elif anamoly == 'Face Mask Detected':
        anamoly = 'face_mask'
    elif anamoly == 'Gun Detected':
        anamoly = 'gun'
    elif anamoly == 'Knife Detected':
        anamoly = 'knife'
    else:
        anamoly = 'no_anamoly'

    # Get class names from model
    class_names = det_model.names  # dict: {0: 'blood', 1: 'face_mask', ...}

    # Find the class index for the specified anamoly
    anamoly_class_id = None
    for cls_id, cls_name in class_names.items():
        if cls_name == anamoly:
            anamoly_class_id = cls_id
            break

    if anamoly_class_id is None:
        print(f"[ERROR] Class '{anamoly}' not found in model.")
        return None, None

    # Filter boxes for that class
    anamoly_boxes = []
    for box in results.boxes:
        if int(box.cls) == anamoly_class_id:
            x1, y1, x2, y2 = box.xyxy[0].cpu().numpy()
            area = (x2 - x1) * (y2 - y1)
            anamoly_boxes.append((area, (x1, y1, x2, y2)))

    if not anamoly_boxes:
        return None, None  # No such object detected

    # Select the biggest box (by area)
    biggest_box = max(anamoly_boxes, key=lambda b: b[0])[1]  # get (x1, y1, x2, y2)

    x1, y1, x2, y2 = biggest_box
    cx = int((x1 + x2) / 2)
    cy = int((y1 + y2) / 2)
    print(f"▶️ Detected {anamoly} at center: ({cx}, {cy})")
    return cx, cy
...
def inference_anamoly(frame , capture_timestamp, intrinsic_matrix , drone_rotation_matrix, drone_pos= [0,0.2,5]):
